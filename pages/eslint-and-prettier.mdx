...
## Additional setup for Backend projects

On backend we use NestJS, thus its eslint config should also be installed.

Install `@darraghor/eslint-plugin-nestjs-typed` [(source)](https://www.npmjs.com/package/@darraghor/eslint-plugin-nestjs-typed)

```bash copy
yarn add -D @darraghor/eslint-plugin-nestjs-typed
```

and add following lines to the config
...
There are also some specific rules which should be turned off or modified, specifically
  ...
  overrides: [
    {
      files: ['**/*.spec.ts'],
      rules: {
        '@typescript-eslint/dot-notation': 'off', // to test private methods in tests
      },
    },
  ]
}
